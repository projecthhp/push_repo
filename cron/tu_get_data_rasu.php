...
if($luatweb!=9){
   ...
   else if($linklogo == 'https://cdn1.mywork.com.vn/company-logo/myworkfb.png' or $linklogo == 'https://cdn1.mywork.com.vn/company-logo-medium/no-logo.png')
   {
      $linklogo = '';
   }
}
...
echo "<hr>";
echo $usc_size            = getValue("usc_size",'int','POST',0);
echo "<hr>";
...
if($new_title != '' && $new_mo_ta != '' && $new_money > 0  && $new_city != 0)
{
   ...
   if(mysql_num_rows($db_qr->result) == 0)
   {
      ...
      if(mysql_num_rows($db_qr7->result) == 0)
      {
         ...
         echo '<hr>Tạo mới company: '.$linklogo.'<hr>';
         $qr = "INSERT INTO user_company(usc_email,usc_name,usc_pass,usc_md5,usc_company,usc_address,usc_phone,usc_logo,usc_create_time,usc_website,usc_size) 
                VALUES('".$new_email_user."','".$new_name_user."','".md5('123viec'.trim($new_email_user))."','".$comp."','".$new_company_cra."','".$new_address_cra."','".$new_phone_user."','".$name."','".time()."','".$new_website."','".$usc_size."')";
      ...
      else
      {
         ...
         if($usc_size != 0)
         {
            if($row['usc_size'] == 0)
            {
               $db_ex11 = new db_execute("UPDATE user_company SET usc_size = '".$usc_size."' WHERE usc_id = '".$id_user."'");
            }
         }
