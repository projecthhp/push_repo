...
function generatePageBar2($page_prefix, $current_page, $page_size, $total_record, $url,$url_prefix = '&', $normal_class, $selected_class,$previous_class = '', $previous='<',$next_class = '', $next='>',$first_class = '', $first='<<',$last_class='', $last='>>', $break_type=1, $page_rewrite=0, $page_space=3, $obj_response='', $page_name="page="){
	...
	if($break_type == 1){
		if(($start_page != 1) && ($num_of_page > 1)){
			...
			$page_bar .=  '<a rel="nofollow" href="' . $href . '"  class="' . $normal_class . ' '.$first_class.'" title="First page">' . $first . '</a> ';
		}
	}
	...
	if($break_type == 1 || $break_type == 2 || $break_type == 4){
		if(($current_page > 1) && ($num_of_page > 1)){
			...
			$page_bar .= ' <a rel="nofollow" href="' . $href . '"  class="' . $normal_class . ' '.$previous_class.'" title="Prev page">' . $previous . '</a> ';
			if(($start_page > 1) && ($break_type == 1 || $break_type == 2)){
				$page_dot_before = $start_page - 1;
            if($current_page == 2)$page_dot_before = '';
				...
				$page_bar .= '<a rel="nofollow" href="' . $href . '" class="' . $normal_class . ' notUndeline">...</a> ';
			}
		}
	}
	...
	if($break_type == 1 || $break_type == 2 || $break_type == 3){
		...
		for($i=$start_loop; $i<=$end_loop; $i++){
			if($i != $current_page){
				...
				$page_bar .= ' <a rel="nofollow" href="' . $href . '"  class="' . $normal_class . '">' . $i . '</a> ';
			}
			else{
				$page_bar .= ' <a rel="nofollow" class="' . $selected_class . '" >' . $i . '</a> ';
			}
		}
	}
	...
	if($break_type == 1 || $break_type == 2 || $break_type == 4){
		if(($current_page < $num_of_page) && ($num_of_page > 1)){
			...
			$page_bar .= ' <a rel="nofollow" href="' . $href . '"  class="' . $normal_class . ' '.$next_class.'" title="Next page">' . $next . '</a> ';
         if(($end_page < $num_of_page) && ($break_type == 1 || $break_type == 2)){
				$page_dot_after = $end_page + 1;
	...
	if($break_type == 1){
		if(($end_page < $num_of_page) && ($num_of_page > 1)){
			...
			$page_bar .= ' <a rel="nofollow" href="' . $href . '"  class="' . $normal_class . ' '.$last_class.'" title="Last page">' . $last . '</a>';
		}
	}
   if($current_page == 2)
   {
