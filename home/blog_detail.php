...
if($newid == 0)
{
   redirect("https://timviec365.com/blog");
}
else
{
   $db_qrdt = new db_query("SELECT * FROM news JOIN categories_multi ON news.new_category_id = categories_multi.cat_id JOIN admin_user ON news.admin_id = admin_user.adm_id  WHERE new_id = ".$newid." LIMIT 1");
   if(mysql_num_rows($db_qrdt->result) == 0)
   {
   	$db_ck = new db_query("SELECT * FROM news WHERE new_id = ".$newid." AND new_category_id = 56");
   	if(mysql_num_rows($db_qrdt->result) == 0){
   		$rck = mysql_fetch_assoc($db_ck->result);
   		$url = "https://timviec365.com/cau-hoi-phong-van/".replaceTitle($rck['new_title_rewrite'])."-pv".$rck['new_id'].".html";
...
if($rowdt['new_category_id']==56){
  ...
  $urlcano = "https://timviec365.com/blog/".replaceTitle($rowdt['new_title_rewrite'])."-new".$rowdt['new_id'].".html";
  $breakcrumb = "/blog/c".$rowdt['new_category_id']."/".replaceTitle($rowdt['cat_link']);
}
...
<!-- End Google Tag Manager (noscript) -->
      ...
      <?include('../includes/blog/inc_header.php');?>
		  ...
		  <?
		     ...
		     $db_ex = new db_execute("UPDATE news SET new_view = new_view+1 WHERE new_id=".$newid."");
		     }
		...
		<div class="breakcrumb blog bg_mota chpv">
			<ul>
				...
				<li itemprop="itemListElement" itemscope="" itemtype="http://schema.org/ListItem" class="active"><a href="<?=$breakcrumb?>"><?= $rowdt['cat_name'] ?></a></li>
			</ul>
		</div>
    <div class="main detail_blog">
      <div class="top_detail">
        ...
        <div class="author_detail">
          <img src="/images/avt_author.png" alt="<?=$rowdt['adm_name']?> tác giả timviec365.com"> 
          <span class="tac_gia">Tác giả: <a href="<?= rewriteTacgia($rowdt['adm_id'],$rowdt['adm_name']) ?>"><?= $rowdt['adm_name']?></a></span>
          <span class="time_dt"><i class="fa fa-clock-o" aria-hidden="true"></i><?= date('d-m-Y',$rowdt['new_date']) ?></span>
        </div>
        <div class="sapo_detail">
          <?
          // 
          $new_teaser = $rowdt['new_teaser'];
          $new_teaser = str_replace('style="text-align: justify;"', '', $new_teaser);
          $new_teaser = str_replace('style="font-family:arial,helvetica,sans-serif;"','',$new_teaser);
          $new_teaser = str_replace('style="font-family: arial, helvetica, sans-serif; font-size: 14px; text-align: justify;"', '',$new_teaser);
          echo $new_teaser; 
          ?>
        </div>
        <a href="/mau-cv.html" ><img style="margin-bottom:15px" src="/images/load.gif" class="lazyload" alt="mẫu cv xin việc" data-src="/images/bannerCvBlog.png"></a>
      </div>
      <div class="muc_luc_blog">
        <?makeML($rowdt['new_description'],'','');?>
        <a id="link_banner_cv" rel="nofollow" target="_blank" href="https://timviec365.vn/cv-xin-viec"><img style="margin:20px 0" src="/images/load.gif" class="lazyload" data-src="/images/bannerBlog2.gif" alt="Tạo CV Online"></a>
      </div>
      <div class="news_des">
        <?
        $new_des = $rowdt['new_description'];
        $new_des = str_replace('style="text-align: justify;"', '', $new_des);
        $new_des = str_replace('style="font-family:arial,helvetica,sans-serif;"','',$new_des);
        $new_des = str_replace('src=', 'class="lazyload" src="/images/load.gif" data-src=',$new_des);
        $sql_lq = '';
        if($rowdt['key_lq'] != ''){
          $arr_vllq = explode(',', $rowdt['key_lq']);
          $sqllq = str_replace(',', "%' OR new_title LIKE '%", $rowdt['key_lq']);
          $sql_lq =  "AND (new_title LIKE '%".$sqllq."%')";                              
        }
        if($sql_lq != ''){
          $sql = "SELECT new_title, new_id,new_alias, usc_logo,usc_company,new_city,new_money,usc_create_time,new_han_nop FROM new INNER JOIN user_company ON new_user_id = usc_id WHERE new_han_nop > '".time()."' ".$sql_lq." ORDER BY RAND() LIMIT 5 ";
          
          $db_vllq = new db_query($sql);
          if(mysql_num_rows($db_vllq->result)>0){
            $main_lq = '';
            $arrcityLq = [];
            While($rowlq = mysql_fetch_assoc($db_vllq->result)){
              $arr_city = explode(',', $rowlq['new_city']);
              foreach ($arr_city as $key => $value) {
                $arrcityLq[] = $arrcity[$value]['cit_name'];
              }
              $txtCityLQ = implode(',', $arrcityLq);
              $main_lq .= 
              '<div class="item main"><div class="logo"><img onerror="this.onerror=null;this.src='."'".'/images/logo-new.png'."'".'"; src="/images/load.gif" class="lazyload" data-src="'.geturlimageAvatar($rowlq['usc_create_time']).$rowlq["usc_logo"].'" alt="'.$rowlq["new_title"].'"></div><a class="title_lq" rel="nofollow" href="'.rewriteNews($rowlq['new_id'],$rowlq['new_title'],$rowlq['new_alias']).'">'.$rowlq["new_title"].'</a><p class="name_company">'.name_company($rowlq["usc_company"]).'</p><div class="money_address"><p class="money"><img src="/images/load.gif" class="lazyload" data-src="/images/ico_money.png" > '.$array_muc_luong[$rowlq['new_money']].'</p></div><div class="city pull-left"><i class="fa fa-map-marker" aria-hidden="true"></i>'.$txtCityLQ.'</div><div class="han_nop"><img class="lazyload" src="/images/load.gif" data-src="/images/ico_time.png">'.date('d/m/Y',$rowlq['new_han_nop']).'</div></div>';
              unset($arrcityLq);
            }
          }
        }
        $html_lq = '<div class="html_lq main"></div>';
        $new_des = str_replace('font-size:14px;', '',$new_des);
        $arr_xt = [];
        echo makeXemthem(button_file(makeML_content($new_des,'','')),$arr_xt,$html_lq);
        ?>
        <?
          if($rowdt['title_suggest']!=''){
        ?>
        <div class="box_suggest main">
          <div class="title_suggest"><div class="iconSuggest"><img src="/images/iconSuggest.png" alt="Icon Suggest"></div><?=$rowdt['title_suggest']?></div>
          <div class="content_suggest">
          <?
          $content_suggest = str_replace('style="text-align: justify;"', '', $rowdt['content_suggest']);
          $content_suggest = str_replace('style="font-size:14px;"', '', $content_suggest);
          $content_suggest = str_replace('style="font-family:arial,helvetica,sans-serif;"','',$content_suggest);
          
          echo $content_suggest;
          ?>
          </div>
        </div>
        <?}?>
        <a rel="nofollow" target="_blank" href="https://timviec365.vn/"><img style="margin:20px 0" src="/images/load.gif" class="lazyload" data-src="/images/bannerBlog.gif" alt="Tìm việc nhanh"></a>
      </div>
      <div class="box_lq">
        <div class="main_lq">
          <div class="head">
            <span class="tag_headding">Bài viết liên quan</span>
          </div>
          <div class="main">
            <?
            $sql_lq = "SELECT new_id, new_title, new_title_rewrite,new_picture, new_date,adm_name,adm_id FROM news JOIN admin_user ON admin_user.adm_id = news.admin_id WHERE new_category_id = ".$rowdt['new_category_id']." AND new_id != ".$rowdt['new_id']." AND new_id < ".$rowdt['new_id']." ORDER BY new_id DESC LIMIT 6 ";
            ...
            ?>
            <div class="item main">
              <div class="image"><img src="/pictures/news/<?=$rowlq['new_picture']?>" alt="<?=$rowlq['new_title']?>"></div>
              <div class="right">
                <a href="<?=rewriteBlog($rowlq['new_id'],$new_title)?>" class="title_lq"><?=$rowlq['new_title']?></a>
                <p class="author_bl"><a href="<?= rewriteTacgia($rowlq['adm_id'],$rowlq['adm_name']) ?>"><?= $rowlq['adm_name']?></a> <?= date('d - m - Y',$rowlq['new_date']) ?></p>
              </div>
            </div>
            <?}unset($dblq,$rowlq)?>
          </div>
      ...
      <div class="box_mxh">
          ...
          <div class="box_bot_dt box_key_related">
            <div class="head_bot_dt head_related">
              <div class="title_bot_dt title_related">Từ khóa liên quan</div>
            </div>
            <div class="body_bot_dt">
            <?
              $new_keyword = $rowdt['new_keyword'];
              $array = ['mô tả','Mô tả','công việc','Công việc','nhân viên','xin việc'];
              $array_replace = ['','','','','',''];
              $sql_key = str_replace($array,$array_replace,$new_keyword);
              $sql_key = str_replace(" ","%' OR new_keyword LIKE '%",trim($sql_key));
              $sql = "SELECT new_keyword, new_title_rewrite,new_title,new_id FROM news WHERE (new_keyword LIKE '%$sql_key%') AND new_id <> $newid ORDER BY new_date DESC LIMIT 30";
              
              $db_keyword = new db_query($sql);
              While($row_keyword= mysql_fetch_assoc($db_keyword->result)){
                $url_title = ($row_keyword['new_title_rewrite']!='')?$row_keyword['new_title_rewrite']:replaceTitle($row_keyword['new_title']);
                echo '<a href="'.rewriteBlog($row_keyword['new_id'],$url_title).'" class="item">'.$row_keyword['new_keyword'].'</a>';
              }
            ?>
            </div>
            <div class="bottom_bot_dt">
              <button class="button"><i class="fa fa-angle-double-down" aria-hidden="true"></i></button>
            </div>
          </div>
          <div class="box_bot_dt box_key_related">
            <div class="head_bot_dt head_related">
              <div class="title_bot_dt title_related">Chuyên mục</div>
            </div>
            <div class="body_bot_dt">
            <?
              $db_cate = new db_query("SELECT cat_name, cat_id, cat_link FROM categories_multi ORDER BY cat_name ASC");
              While($rowCate = mysql_fetch_assoc($db_cate->result)){
                $url = ($rowCate['cat_link']!='')?$rowCate['cat_link']:replaceTitle($rowCate['cat_name']);
                $url = "/blog/c".$rowCate['cat_id']."/".$url;
                echo '<a href="'.$url.'" class="item">'.$rowCate['cat_name'].'</a>';
              }
            ?>
            </div>
            <div class="bottom_bot_dt">
      ...
      <div class="comment">
          ...
          <div class="form_comment main">
            ...
            <i class="fa fa-refresh fa-2x reload hidden" onclick="reloadSecurityCode(this)" aria-hidden="true"></i>
            <div class="clear"></div>
                ...
                <div class="item_show main">
                  <div class="comment_parent">
                    ...
                    <div class="detail">
                      ...
                      <p class="content"><?= removeHTML($row_cm['cmt_content']) ?></p>
                      <a onclick="replyComment(`<?= $row_cm['cmt_id']?>`)" class="reply">Phản hồi</a>
                    </div>
                    ...
                    ?>
                      <li>
                        <?
                          ...
                          ?>
                        <div class="img">
                          <img src="/images/load.gif" class="lazyload" data-src="<?=$src_rep?>" alt="Logo user reply">
                        </div>
                        <div class="detail">
                          <p class="username">
      ...
      <a style="display:inline-block;width:100%" href="/"><img style="margin-bottom:15px" src="/images/load.gif" class="lazyload" data-src="/images/banner_timviec.png" alt="Tìm việc làm"></a>
      </div>
        ...
        <!-- <a href="/"><img style="margin-bottom:15px" src="/images/load.gif" class="lazyload" data-src="/images/banner_timviec.png" alt="Tìm việc làm"></a> -->
      ...
      <?if(isset($main_lq)){?>
        ...
        $(document).ready(function(){          
          ...
          txt = '<div class="panel-heading"><i class="fa fa-briefcase" aria-hidden="true"></i> Việc làm hấp dẫn <i class="fa fa-bars" aria-hidden="true" onclick="hideShowContent(this)"></i></div>';
          txt += '<div class="panel-body main">'+`<?=$main_lq?>`+'</div>';
