...
<?php 
  ...
  if(isset($_COOKIE['UID'])){
    if($_COOKIE['UT']==0){
      ...
      $use_address = mysql_fetch_assoc($qr_user->result)['use_address'];
    }else{
...
<head>
 ...
 <title>Tìm ứng viên quanh đây</title>
  ...
  <header>
    ...
    ?>
      ...
      <div class="text-center"> 
        ...
        <a class="downLoad_App CV365" href="/ung-dung-tao-cv.html" rel="nofollow"><i class="icon"></i>Tải app CV365</a>
      </div>
    </div>
      </header>
  </div>
  <div id="map-vl">
    <p class="td">tìm ứng viên quanh đây</p>
...
var markerGroups = { "icon1": [], "icon2": [], "icon3": [], "icon11": [], "icon21": [], "icon31": []};
...
function initMap() {    
  ...
  if(job != '' || cate > 0 || km > 0){
    if(job!=''){
      var url = "home/rasu_map_uv.php?code=" + code + '&lat=' + valat + '&long=' + valng + "&job=" + job + "&cate=" + cate + '&km=' + km;
    }else{
      var url = "home/rasu_map_uv.php?code=" + code + '&lat=' + valat + '&long=' + valng + "&cate=" + cate + '&km=' + km;
    }
  }else{    
    var url = "home/rasu_map_uv.php?code=" + code + '&lat=' + valat + '&long=' + valng + '&km=2';    
  }      
  console.log(url);
...
function geocodeAddress(geocoder, resultsMap, met) {
  ...
  geocoder.geocode({'address': address}, function(results, status) {
    if (status === 'OK') {
      ...
      alert('Bạn cần nhập địa chỉ chi tiết hơn');
    }
  });
}
