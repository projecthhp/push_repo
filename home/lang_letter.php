...
$listseo = new db_query("SELECT new_lang_letter.*,languagecv.alias,languagecv.name as name_lang FROM new_lang_letter JOIN languagecv ON new_lang_letter.id_lang = languagecv.id WHERE new_lang_letter.id_lang = '".$langid."' ");

if(mysql_num_rows($listseo->result) == 0){
    redirect('/home/404.php');
}
$rowseo = mysql_fetch_assoc($listseo->result);
$alias_url = $rowseo['alias'];

$canonical = "https://timviec365.com/mau-thu-xin-viec/thu-xin-viec-".$alias_url."-s".$langid.".html";
$check_url = "/mau-thu-xin-viec/thu-xin-viec-".$alias_url."-s".$langid.".html";
...
if($rowseo['meta_tit']!=''){
  ...
  $seo_tt = 'Thư xin việc theo ngôn ngữ';
}
...
<div class="breakcrumb">
  <ul>
    ...
    <li class="active"><a>Thư xin việc <?=strtolower($rowseo['name_lang'])?></a></li>
  </ul>
</div>
  ...
  <div class="box_text_seo">
    ...
    <div class="content">
        ...
        $content = str_replace('style="font-family:arial,helvetica,sans-serif;"','',$content);
          echo makeML_content(str_replace('src=', 'class="lazyload" src="/images/load.gif" data-src=', $content),'','')?></div>
        <div class="read">
