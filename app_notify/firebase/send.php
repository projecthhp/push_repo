...
<body>
    ...
    foreach ($arrCity as $cityId) {
        foreach ($arrCareer as $careerId) {
            //        $time_update = time() - 10800;
            $time_update = time() - $count_time;
            //        echo "time: ".$count_time;
            $sql = "SELECT new_id FROM new WHERE new_city=" . $cityId . " AND new_cat_id=" . $careerId . " AND new_update_time >= " . $time_update . " LIMIT 30";
            //                echo $sql;
            //                exit();
            ...
            if ($result->num_rows > 0) {
                ...
                $filters = array(array("field" => "tag", "key" => "usertype", "relation" => "=", "value" => "uv"), array("operator" => "AND"), array("field" => "tag", "key" => "cit", "relation" => "=", "value" => "cit_$cityId"), array("operator" => "AND"), array("field" => "tag", "key" => "cat", "relation" => "=", "value" => "cat_$careerId"));
                $filters = json_encode($filters);
                // $topic = "'cit_$cityId' in topics && 'cat_$careerId' in topics && 'ung_vien' in topics";
                $sqlInsert = "INSERT INTO send_notification(filters,type,status,time_start,time_end,count) VALUES ('" . $filters . "',1,0," . $time_start . "," . $time_end . "," . $count . ")";
                // echo $sqlInsert;
                // die();
    ...
    $sqlSend = "SELECT * FROM send_notification";
