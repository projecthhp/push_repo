...
{
   $db_qrcheck = new db_query("SELECT use_id FROM user WHERE use_id = '".$userid."' AND use_pass = '".$old_pass."' LIMIT 1");
   if(mysql_num_rows($db_qrcheck->result) > 0)
   {
      if($old_pass != '')
      {
         $db_ex1 = new db_execute("UPDATE user SET use_pass = '".$new_pass."' WHERE use_id = '".$userid."'");
