...
if($action == "submitForm"){
   ...
   $errorMsg .= $myform->checkdata();
    ...
    $check = new db_query("SELECT count(*) FROM ".$fs_table." WHERE cat_id = $cat_id AND cit_id = $cit_id ");
  $numrow = mysql_fetch_array($check->result);

  if ($numrow['count(*)']) {
    $errorMsg .= "Đã tồn tại bài viết";
  }
	if($errorMsg == ""){
    ...
    <td class="form_text">
        ...
        $db_cat = new db_query("SELECT * FROM category WHERE cat_active = 1 ORDER BY cat_id ASC");
            while($row_cat = mysql_fetch_array($db_cat->result)){         
                ?>
                    <option <?= ($cat_id==$row_cat['cat_id'])?"selected":"" ?> value="<?=$row_cat["cat_id"]?>" >
                    <?
      ...
      <?
       $db_cit = new db_query("SELECT * FROM city ORDER BY cit_id ASC");
          while($row_cit = mysql_fetch_array($db_cit->result)){  
                ?>
                    <option <?= ($cit_id==$row_cit['cit_id'])?"selected":"" ?> value="<?=$row_cit["cit_id"]?>" >
                    <?
...
<?=$form->close_table();?>
<?=$form->wysiwyg("Nội dung chi tiết", "meta_content", $meta_content, "../../resource/ckeditor/", "100%","600")?>
<?=$form->create_table();?>
<?=$form->text("SEO Title", "meta_tit", "meta_tit", $meta_tit, "SEO Title",1,500,25,255)?>
<?=$form->text("SEO Key", "meta_key", "meta_key", $meta_key, "SEO Key",1,500,25,255)?>
<?=$form->textarea("SEO Description", "meta_des", "meta_des", $meta_des, "SEO Description", 1, 500, 120, "", "", "")?>
