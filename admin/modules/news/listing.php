...
function rewriteBlog($cate,$id,$title){
  if($cate == 56){
...
$db_listing = new db_query("SELECT * 
									 ...
									 JOIN categories_multi ON ".$fs_table.".new_category_id = categories_multi.cat_id
									 WHERE 1 " . $sql . "
   ...
   while($row = mysql_fetch_assoc($db_listing->result)){
      ...
      <?=$list->start_tr($i, $row[$id_field]);?>
         <td align="center" width="72">
			...
			?>
         <td><a href="https://timviec365.com<?=rewriteBlog($row['new_category_id'],$row['new_id'],replaceTitle($title_rewrite))?>" target="_blank"><?=$row['new_title']?></a></td>  
         <td align="center"><?= $new_category_id[$row['new_category_id']] ?></td>          
