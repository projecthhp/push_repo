<?php
    ...
    */
    ...
    $time = time();
    $nexttime = mktime(0,0,0,date('m'),date('d')+7,date('Y'));

    $db_full = new db_query("SELECT * 
                                         FROM " . $fs_table . "
                                         JOIN user_company_multi ON ".$fs_table.".usc_id = user_company_multi.usc_id
                                         LEFT JOIN new ON user_company.usc_id = new.new_user_id
                                         WHERE 1 " . $sql . " AND (usc_alias <> '' OR register = 2) AND new_han_nop BETWEEN ".$time." AND ".$nexttime."
                                         GROUP BY user_company.usc_company ORDER BY " . $list->sqlSort() .$fs_table.".". $field_id . " DESC ");
    $total_row = mysql_num_rows($db_full->result);
    ...
    $db_listing = new db_query("SELECT * 
                                         ...
                                         LEFT JOIN new ON user_company.usc_id = new.new_user_id
                                         WHERE 1 " . $sql . " AND (usc_alias <> '' OR register = 2) AND new_han_nop BETWEEN ".$time." AND ".$nexttime."
                                         GROUP BY user_company.usc_id ORDER BY " . $list->sqlSort() .$fs_table.".". $field_id . " DESC " .
