<?php
require_once("inc_security_interview.php");
$iAdm=0;
...
$list->add("position","Vị trí phỏng vấn","date",1,0, 'width="20%"');
$list->add("description","Quá trình phỏng vấn","string",0,0, 'width="100"');

$list->add("questions", "Câu hỏi phỏng vấn", "string", 1, 0);
$list->add("answers", "Câu trả lời ", "string", 0, 0);
...
<div id="listing">
   ...
   while($row = mysql_fetch_assoc($db_listing->result)){
	  ...
	  ?>
		 ...
		 <td align="center"><?=$row['answers']?></td> 
         <?=$list->showCheckbox("rate_active_interview",$row['rate_active'],$row['rate_id'])?>   
         <td width="10" align="center"><a class="delete" href="#" onclick="if (confirm('Bạn muốn xóa bản ghi?')){ deleteone(<?=$row['rate_id']?>); }"><img src="../../resource/images/grid/delete.gif" border="0"></a></td>
      <?=$list->end_tr();?>
      <?
   }
   ?>
