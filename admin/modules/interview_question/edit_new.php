...
$myform->add("new_title_rewrite","new_title_rewrite",0, 1,"",1,"Bạn chưa nhập URL",0,"");
...
$myform->add("new_keyword","new_keyword",0,0,"",0,"Bạn chưa keyword",0,"");
$myform->add("new_hot","new_hot",1,0,0,0,"",0,"");
$myform->add("new_301","new_301",0,0,0,0,"",0,"");
    ...
    <td class="form_text">
        <select name="pro_category_id" id="pro_category_id" class="form_control">
            ...
            foreach($listAll as $i=>$cat){
                ?>
                    <option <?=($i == $row['new_category_id'])?"selected":""?> value="<?=$i?>" >
                    <?
                     echo $cat;
                    ?>
                    </option>
...
<tr>
    <td class="form_name">Bài viết liên quan</td>
    <td class="form_text">
         <select id="new_url_lq"  class="new_url_lq" name='new_url_lq[]' multiple="" data-select2-id="3" tabindex="-1" aria-hidden="true">
            <option value="">Chọn bài viết</option>
            <?

            $db_qr = new db_query("SELECT new_id,new_title FROM new_chpv WHERE new_active = 1");

            While($rows = mysql_fetch_assoc($db_qr->result))
            {
            ?>
            <option value="<?= $row['new_id'] ?>"><?= $rows['new_title'] ?></option>
            <?
            }
            unset($type,$item);
            ?>
...
<?=$form->text("Tiêu đề tin","new_title","new_title",$row['new_title'],"Tiêu đề tin",1,500,"",255)?>
<?=$form->text("URL","new_title_rewrite","new_title_rewrite",$row['new_title_rewrite'],"URL",0,500,"",255)?>
<?=$form->text(translate_text("Thứ tự"),"new_order","new_order",$row['new_order'],translate_text("Thứ tự"),0,50,"",255);?>

<?=$form->getFile("Ảnh minh họa", "picture", "picture", "Ảnh minh họa", 0, 32, "", '<br />(Dung lượng tối đa <font color="#FF0000">'.$limit_size.' Kb</font>)')?>
<?=$form->text("Title (<span id='count_title'>0</span>/65 ký tự)","new_tt","new_tt",$row['new_tt'],"Title",0,500,"",255)?>

<?=$form->textarea("Description (<span id='count_description'>0</span>/160 ký tự)", "new_des", "new_des", $row['new_des'], "Description", 0, 500, 60, "", "", "")?>
<?=$form->textarea("Keywords", "new_keyword", "new_keyword", $row['new_keyword'], "Keywords", 0, 600, 120, "", "", "")?>
<?=$form->text("Link 301","new_301","new_301",$row['new_301'],"Link 301",0,500,"",255)?>

<?=$form->close_table();?>
<?=$form->wysiwyg("Tóm tắt", "new_teaser", $row['new_teaser'], "../../resource/ckeditor/", "80%","600")?>
<?=$form->create_table();?>
...
<?=$form->close_table();?>
<?=$form->wysiwyg("Nội dung chi tiết", "new_description", $row['new_description'], "../../resource/ckeditor/", "100%","600")?>
<?=$form->create_table();?>
