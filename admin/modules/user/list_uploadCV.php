...
$enddate       = getValue("enddate", "str", "GET", "dd/mm/yyyy");
   ...
   $total         = new db_count("SELECT count(*) AS count 
                                  FROM " . $fs_table . "
                                  JOIN user_cv_upload                              
                                  ON user_cv_upload.use_id = user.use_id
                                  WHERE 1 AND use_show = 0 " . $sql);                           
   $db_listing = new db_query("SELECT * 
                               FROM " . $fs_table . "
                               JOIN user_cv_upload                              
                               ON user_cv_upload.use_id = user.use_id
                               WHERE 1 AND use_show = 0 " . $sql . "
                               ORDER BY " . $list->sqlSort() .'user.'. $field_id . " DESC
...
<?=$list->showHeader($total_row)?>
   ...
   while($row = mysql_fetch_assoc($db_listing->result)){
      ...
      <?=$list->start_tr($i, $row['use_id']);?>     
         ...
         <?
            $arr_link = explode('.', $row['link']);
            $type_link = end($arr_link);
            $arr_ckeck_type = array('jpg','jpeg','png','PNG','gif','jfif','pdf');
            if(!in_array($type_link, $arr_ckeck_type)){
               $img = str_replace('../','https://docs.google.com/viewer?url=timviec365.com/',$row['link']);
            }
            else{
               $img = str_replace('../','https://timviec365.com/',$row['link']);
            }
         ?>
         <td style="text-align: center;"><a target="_blank" href="<?= $img ?>">CV</a></td>
