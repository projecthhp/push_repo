...
$enddate       = getValue("enddate", "str", "GET", "dd/mm/yyyy");
   ...
   $list->add("use_update_time", "Ngày cập nhật", "string",0,0);
   ...
   if($enddate != "dd/mm/yyyy"){
	...
	$db_listing = new db_query("SELECT * 
										 FROM " . $fs_table . "
										 WHERE 1 " . $sql . "
										 ORDER BY use_update_time DESC, use_id DESC
...
<?=$list->showHeader($total_row)?>
   ...
   while($row = mysql_fetch_assoc($db_listing->result)){
      ...
      <?=$list->start_tr($i, $row['use_id']);?>     
         ...
         <td style="text-align: center;"><?=($row['use_update_time']>0)?date('d/m/Y',$row['use_update_time']):date('d/m/Y',$row['use_create_time'])?></td>   
