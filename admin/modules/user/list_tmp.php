...
$enddate       = getValue("enddate", "str", "GET", "dd/mm/yyyy");
   ...
   $list->add("tmp_register", "Nguồn ĐKý", "string",0,0);
   ...
   if($enddate != "dd/mm/yyyy"){
	...
	$total			= new db_count("SELECT count(*) AS count 
											 FROM " . $fs_table . "
											 WHERE 1 AND (tmp_register = 1 OR tmp_register = 3) " . $sql);									 
	$db_listing = new db_query("SELECT * 
										 FROM " . $fs_table . "
										 WHERE 1 AND (tmp_register = 1 OR tmp_register = 3) " . $sql . "
										 ORDER BY " . $list->sqlSort() . $field_id . " DESC
										 " . $list->limit($total->total));
   $db_full = new db_query("SELECT * FROM ".$fs_table." WHERE 1 AND (tmp_register = 1 OR tmp_register = 3) ".$sql." ORDER BY " . $list->sqlSort() . $field_id . " DESC ");
...
<form method="post" name="form_ab">
   ...
   while($row = mysql_fetch_assoc($db_listing->result)){
      ...
      <?=$list->start_tr($i, $row['tmp_id']);?>
         ...
         <td style="text-align: center;"><?=$arr_tmp[$row['tmp_register']]?></td>   
