...
$myform->add("use_mail","use_mail",0, 0,"",1,"Bạn chưa nhập Email ứng viên",1,"Email đã tồn tại");
$myform->add("use_name","use_name",0, 0,"",1,"Bạn chưa nhập tên ứng viên",0,"");
$myform->add("use_phone","use_phone",1, 0,"",1,"Bạn chưa nhập số điện thoại",0,"");
...
$myform->add("work_option","work_option",0, 0,"",1,"Bạn chưa chọn mức lương",0,"");
...
$myform->add("use_time","use_time",0, 0,"",0,"",0,"2");
...
<tr>
    ...
    <td class="form_text">
        <input type="date" id="birthday" name="birthday" value="<?= ($birthday != '')?$birthday:"" ?>" style="width: 250px;"/>
...
<tr>
    ...
    <td class="form_text">
        <select name="gender" id="gender" class="form_control" style="width: 250px;">
            ...
            ?>
            <option <?= ($gender == $key)?'selected':'' ?> value="<?= $key ?>"><?= $val ?></option>
            <?
            }
            ?>
        </select>
...
<tr>
    ...
    <td class="form_text">
        <select name="honnhan" id="honnhan" class="form_control" style="width: 250px;">
            ...
            ?>
            <option <?= ($honnhan == $key)?'selected':'' ?> value="<?=$key?>"><?= $val ?></option>
            <?
            }
            ?>
        </select>
...
<tr>
    ...
    <td class="form_text">
        <select name="use_city" id="use_city" class="form_control">
        	<option value="">Chọn thành phố</option>
			<?
			$db_qr = new db_query("SELECT cit_id, cit_name FROM city");
            While($row = mysql_fetch_array($db_qr->result)){
            ?>
            <option <?= ($city == $row['cit_id'])?"selected":""?> value="<?=$row['cit_id']?>" >
            <?
            	echo $row['cit_name'];
            ?>
           	</option>
...
<tr>
    ...
    <td class="form_text">
        <select name="cap_bac_mong_muon" id="cap_bac_mong_muon" class="form_control" style="width: 250px;">
            ...
            ?>
            <option <?= ($cap_bac_mong_muon == $key)?"selected":"" ?> value="<?=$key?>"><?= $val ?></option>
            <?
            }
            ?>
        </select>
...
<tr>
    ...
    <td class="form_text">
        <select name="use_district_job[]" id="use_district_job" class="form_control" multiple="">
            ...
            <?
                if(!empty($use_district_job)){
                    if(in_array($row['cit_id'], $use_district_job)){
                        echo "selected";
                    }
                }
        ...
        <select name="use_nganh_nghe[]" id="use_nganh_nghe" class="form_control" multiple="">
            ...
            <?
                if(!empty($use_nganh_nghe)){
                    if(in_array($row['cat_id'], $use_nganh_nghe)){
                        echo "selected";
                    }
                }
...
<!-- <tr>
    <td class="form_name"><font class="form_asterisk">* </font>Bằng cấp:</td>
    <td class="form_text">
        <select name="rank" id="rank" class="form_control" style="width: 250px;">
			<?
            foreach ($array_xl as $key => $val) {
            ?>
			<option <?= ($rank == $key)?"selected":"" ?> value="<?=$key?>"><?= $val ?></option>
            <?
            }
            ?>
        </select>
...
<?=$form->textarea("Mục tiêu nghề nghiệp","muc_tieu_nghe_nghiep","muc_tieu_nghe_nghiep",$muc_tieu_nghe_nghiep,"Mục tiêu làm việc",1,500,120,"")?>
...
<!-- <?=$form->text("Lat ","lat","lat",$lat,"lat",0,250,"",255)?>
<?=$form->text("Long ","lat","lat",$school_name,"school_name",0,250,"",255)?> -->
