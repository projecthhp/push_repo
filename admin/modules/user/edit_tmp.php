...
<?
require_once("inc_security_tmp.php");
...
$myform->addTable('user');	// Add table
#+
...
$myform->add("use_pass","use_pass",0, 0,"",0,"",0,"1");
$myform->add("use_authentic","use_authentic",0, 0,"3",0,"",0,"4");
$myform->add("use_create_time","use_create_time",0, 0,"",0,"",0,"5");
...
if($action == "submitForm"){
   ...
   if($array_config["image"]==1){
	...
	if($errorMsg == ""){
        ...
        $update = new db_query("UPDATE user SET use_district_job = '".$use_district_job."', use_nganh_nghe = '".$use_nganh_nghe."' WHERE use_id = ".$last_id." ");
        ...
        if($tg_batdau_hoc != ''){
            ...
            $last_id1       = $db_ex->db_execute($query1);
            $update1 = new db_query("UPDATE user_hocvan SET use_id = '".$last_id."' WHERE id_hocvan = ".$last_id1." ");
        }
        ...
        if($id_ngonngu != ''){
            ...
            $update3 = new db_query("UPDATE use_ngoaingu SET use_id = '".$last_id."' WHERE id_ngoaingu = ".$last_id3." ");
        }

        $delete = new db_query("DELETE FROM ".$fs_table." WHERE ".$field_id." = ".$record_id." ");
...
<?=$form->text("Email ứng viên","use_mail","use_mail",$tmp_email,"Email ứng viên",1,250,"",255)?>
<?=$form->text("Tên ứng viên","use_name","use_name",$tmp_name,"Tên ứng viên",1,250,"",255)?>
<?=$form->text("Số điện thoại","use_phone","use_phone",$tmp_phone,"Số điện thoại",1,250,"",255)?>
<tr>
    <td class="form_name"><font class="form_asterisk">* </font>Ngày sinh:</td>
    <td class="form_text">
        <input type="date" id="birthday" name="birthday" value="" style="width: 250px;" />
         <input type="hidden" id="use_pass" name="use_pass" value="<?= md5('timviec365') ?>" />
        <input type="hidden" id="use_time" name="use_time" value="<?= time() ?>" />
        <input type="hidden" id="use_authentic" name="use_authentic" value="<?= 1 ?>" />
        <input type="hidden" id="use_create_time" name="use_create_time" value="<?= time() ?>" />
...
<tr>
    ...
    <td class="form_text">
        <select name="use_city" id="use_city" class="form_control" style="width: 250px;">
            ...
            ?>
            <option <?= ($row_get['tmp_city'] == $row['cit_id'])?"selected":""?> value="<?=$row['cit_id']?>" >
            <?
            	echo $row['cit_name'];
            ?>
           	</option>
...
<tr>
    ...
    <td class="form_text">
        <select name="use_district" id="use_district" class="form_control" style="width: 250px;">
            ...
            <?
            $db_qh = new db_query("SELECT * FROM city2 WHERE cit_parent = ".$row_get['tmp_city']." ");
            while($rqh = mysql_fetch_array($db_qh->result)){
            ?>
            <option <?= ($row_get['tmp_distric'] == $rqh['cit_id'])?"selected":"" ?> value="<?= $rqh['cit_id']?>"><?= $rqh['cit_name']?></option>
            <?
            }
            ?>
        </select>
...
<?=$form->textarea("Địa chỉ chi tiết","address","address",$tmp_address,"Địa chỉ chi tiết",1,250,50,"")?>
<?=$form->text("Công việc mong muốn","use_job_name","use_job_name",$tmp_job_name,"Công việc mong muốn",1,250,"",255)?>
<tr>
...
<tr>
    ...
    <td class="form_text">
        <select name="use_district_job[]" id="use_district_job" class="form_control" multiple="">
            ...
            <option
            <?
                $arr_tt = explode(',', $row_get['tmp_job_city']);
...
<tr>
    ...
    <td class="form_text">
        <select name="use_nganh_nghe[]" id="use_nganh_nghe" class="form_control" multiple="">
            ...
            <option 
            <?
                $arr_nn = explode(',', $row_get['tmp_nganh_nghe']);
...
<?=$form->textarea("Thông tin bổ sung","thongtin_bosung","thongtin_bosung",$thongtin_bosung,"Thông tin bổ sung",0,500,120,"")?>
...
<?=$form->textarea("Thêm thông tin","them_thongtin","them_thongtin",$them_thongtin,"Thêm thông tin",0,500,120,"")?>
<tr>
