...
$enddate       = getValue("enddate", "str", "GET", "dd/mm/yyyy");
   ...
   $list->add("new_title", "Việc làm", "string",0,0);
   $list->add("usc_company", "Tên công ty", "string",0,0);
   $list->add("usc_phone", "SĐT công ty", "string",0,0);
   $list->add("usc_email", "Mail công ty", "string",0,0);
	$list->add("nhs_time", "Ngày nộp", "string",0,0);
...
<?=$list->showHeader($total_row)?>
   ...
   while($row = mysql_fetch_assoc($db_listing->result)){
      ...
      <?=$list->start_tr($i, $row['id']);?>     
         ...
         <td style="text-align: center;"><?=$row['usc_email']?></td>
         <td style="text-align: center;"><?=date('d/m/Y',$row['nhs_time'])?></td> 
         <?=$list->showEdit($row['use_id'])?>        
         <?=$list->showDeleteUT($row['id'])?>                      
      <?=$list->end_tr();?>
      <?
   }
   ?>
