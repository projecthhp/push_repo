...
$myform->add("usc_id","usc_id",0, 0,"",1,"Bạn chưa nhập id công ty",1,"ID công ty đã tồn tại");
$myform->add("point","point",0, 0,"",1,"Bạn chưa nhập số điểm khuyến mãi",0,"");
$myform->add("point_usc","point_usc",0, 0,"",1,"Bạn chưa nhập số điểm cộng thêm",0,"");
$myform->add("day_reset_point","day_reset_point",1, 0,"",0,"",0,"");

// $myform->add("name","name",0, 0,"",1,"Bạn chưa nhập tên tag",0,"");
// $myform->add("point","point",0, 0,"",1,"Bạn chưa chọn tên tỉnh thành",0,"");
...
<? 
    $point_fr = "SELECT point FROM tbl_point
                       WHERE point_id = 1";
    $db_fr  = new db_query($point_fr);
    $row_fr   = mysql_fetch_assoc($db_fr->result);
?>

<?=$form->text("Nhập ID công ty","usc_id","usc_id",$usc_id,"id",1,300,"",255)?>
<?=$form->text("Nhập số điểm khuyến mãi ","point","point",$row_fr['point'],"điểm",1,300,"",255,'','readonly')?>
<?=$form->text("Nhập số điểm mất phí","point_usc","point_usc",$point_usc,"điểm",1,300,"",255)?>
<input type="hidden" value="<?=time()?>" name="day_reset_point" />
