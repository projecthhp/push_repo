...
$myform->add("name","name",0, 0,"",1,"Bạn chưa nhập Tên mẫu CV",0,"");
$myform->add("alias","alias",0, 0,"",1,"Bạn chưa nhập tên CV (không dấu)",0,"");
$myform->add("timecreated","timecreated",1,0,$timecreated,0,"",0,"");
$myform->add("idnganh","idnganh",0, 0,"",1,"Bạn chưa chọn ID ngành nghề",0,"");
$myform->add("idlang","idlang",0, 0,"",1,"Bạn chưa nhập ID ngôn ngữ",0,"");
...
$myform->add("codecolor","codecolor",0, 0,"",0,"Bạn chưa chọn mã màu",0,"");
$myform->add("serial","serial",0, 0,"",0,"",0,"");
$myform->add("status","status",0, 0,"",0,"",0,"");
$myform->add("type","type",0, 0,"",0,"",0,"");
$myform->add("price","price",0, 0,"",0,"",0,"");
...
if($action == "submitForm"){
  ...
  if($errorMsg == ""){
//     if (isset($_POST['htmlcss_vi'])) {
//       $htmlcss_vi = $_POST['htmlcss_vi'];
//       $htmlcss_en = $_POST['htmlcss_en'];
//       $htmlcss_jp = $_POST['htmlcss_jp'];
//       $htmlcss_cn = $_POST['htmlcss_cn'];
//       $htmlcss_kr = $_POST['htmlcss_kr'];
// }
    #+
    #+ Thuc hien query
    ...
    $db_ex = new db_execute("UPDATE samplecv SET htmlcss_vi = '".$htmlcss_vi."',htmlcss_en = '".$htmlcss_en."',htmlcss_jp = '".$htmlcss_jp."',htmlcss_cn = '".$htmlcss_cn."',htmlcss_kr = '".$htmlcss_kr."' WHERE id = ".$last_id."");
...
<!-- <?=$form->text("ID danh mục","idcate","idcate",$idcate,"ID danh mục",1,600,30,255)?> -->
<tr>
...
<?=$form->text("Tên CV","name","name",$name,"Nhập tên mẫu CV",1,600,30,255)?>
<?=$form->text("Tên CV(không dấu)","alias","alias",$alias,"Nhập tên CV không dấu",1,600,30,255)?>
<?=$form->getFile("Ảnh đại diện","image","image","Ảnh đại diện", 0, 32, "", '(Dung lượng tối đa <font color="#FF0000">' . $limit_size . ' Kb</font>)');?>
<!-- <?=$form->text("ID ngành nghề","idnganh","idnganh",$idnganh,"chọn ID ngành nghề",1,600,30,255)?> -->
<tr>
    <td class="form_name"><font class="form_asterisk">* </font>Chọn ngành nghề</td>
    <td class="form_text">
    <select name="idnganh" id="idnganh">
      <option value="0">Chọn ngành nghề CV</option>
      <?
      foreach ($arr_catecv as $key => $value) {
      ?>
...
<?=$form->text("ID ngôn ngữ","idlang","idlang",$idlang,"Chọn ID ngôn ngữ",1,600,30,255)?>
...
<?=$form->text("Thứ tự","serial","serial",$serial,"Chọn thứ tự",1,600,30,255)?>
<?=$form->text("Trạng thái","status","status",$status,"Trạng thái hiển thị",0,600,30,255)?>
<?=$form->text("Loại CV","type","type",$type,"loại",0,600,30,255)?>
<?=$form->text("Giá tiền","price","price",$price,"Nhập giá tiền",0,600,30,255)?>
<?=$form->text("meta_title","meta_title","meta_title",$meta_title,"Dành cho robot đọc",0,600,30,255)?>
<?=$form->text("meta_key","meta_key","meta_key",$meta_key,"Dành cho robot đọc",0,600,30,255)?>
<?=$form->text("meta_desciption","meta_desciption","meta_desciption",$meta_desciption,"Dành cho robot đọc",0,600,30,255)?>
