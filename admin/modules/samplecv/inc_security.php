...
$db_catecv = new db_query("SELECT * FROM nganhcv");
while($rowcv = mysql_fetch_array($db_catecv->result)){
	$arr_catecv[$rowcv['id']] = $rowcv;
}

$db_dmcv = new db_query("SELECT * FROM danhmuccv");
while($rowdm = mysql_fetch_array($db_dmcv->result)){
	$arr_dmcv[$rowdm['id']] = $rowdm;
}
?>